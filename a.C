...
const static char FIRST_CHAR = 32; //
const static char LAST_CHAR = 253; // 
const static int F_TABLE_SIZE = 26;
const static float PT_COMPARE = get<1>(PT[8]);
const static float ENG_COMPARE = get<1>(ENG[24]);
const static float UNLIKELY_KEYS = {
    0
};
...
float shifted_freq( vector<pair<char,float>> data, int shift,vector<pair<char,float>> table) {
    ...
    vector<pair<char,float>> diffComparison;
    
     while (i < F_TABLE_SIZE && turnAround < sizeData) 
    {
        importance = 1;
        if(i < F_TABLE_SIZE -1){
            importance = abs(get<1>(table[i]) - get<1>(table[i + 1])); // when i is peak index
            importance *= importance;
        }
        if(i > 0){
            importance += abs(get<1>(table[i]) -get<1>(table[i - 1]));
            importance *= importance;
        }
        ...
        if(shift + i < sizeData){
            ...
            diff *= get<1>(table[i]);
           /*  if(shift + i + 1 < sizeData){
                diff += abs(diff - get<1>(data[i + shift + 1])); // a correction factor
            } */
        }
        else
        {
            ...
            diff *= get<1>(table[i]);
          /*   if(turnAround + 1 < sizeData){
                diff += abs(diff - get<1>(data[turnAround+ 1])); // a correction factor
            } */
        }
        
        diffSum += diff*importance;
        diffComparison.push_back( make_pair(get<0>(table[i]), diff));
        i++;
    } 
...
int main(){
   ...
   data = read_from_in("in.txt"); 
    ...
    cout << endl << break_vigenere("Three days of rest, three blessed days of rest, are what I had with Wolf Larsen, eating at the cabin  able and doing nothing but discuss life, literature, and the universe, the while  Hi my name is slim shady beak it crawling pieces of somewjer beyond the rainbow", key.size(), ENG );
    //cout << enc;
